...
class BrightnessController:
    def __init__(self, camera_index=0, num_threads=4, frame_queue_size=100, brightness_queue_size=100,
        ...
        self.running = True  # Добавяне на променлива за контрол на цикъла
    ...
    def get_screenshot_brightness(self):
        while self.running:  # Проверка на променливата за контрол на цикъла
            try:
                ...
                save_state((self.prev_brightness, self.smoothed_brightness,
    ...
    def adjust_screen_brightness(self):
        ...
        try:
            ...
            # Прекратяване на циклите при получаване на прекъсване от клавиатурата
            self.running = False
        finally:
            for _ in range(self.num_threads):
                frame_queue.put(None)
            screenshot_thread.join()  # Изчакване на нишката да приключи
            if cap is not None:
                cap.release()
            cv2.destroyAllWindows()
