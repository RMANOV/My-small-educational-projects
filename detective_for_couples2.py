...
def get_paired_info(data, paired_devices):
    ...
    for dev in data:
        ...
        if paired_with:
            paired_info.append((dev["user"], dev["first"], dev["last"], dev["count"], dev["mac"], dev["ip"], paired_with))
    return paired_info
...
def detective(user):
    ...
    detected_devices = [(user, first, last, count, mac, ip, paired) for user, first, last, count, mac, ip, paired in paired_info if user == user and any(pair["user"] == user for pair in paired)]
