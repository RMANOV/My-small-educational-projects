...
def process_frames(frame_queue, brightness_queue):
    while True:
...
def adjust_screen_brightness(camera_index=0, num_threads=4):
                ...
                if prev_time is None or (current_time - prev_time).total_seconds() >= debounce_time:
                    try:
                        ...
                        print(f'New brightness: {math.ceil(smoothed_brightness)}% at {current_time.strftime(
                            "%H:%M:%S")}, debounce time: {debounce_time:.2f}, smoothing factor: {smoothing_factor:.2f}')
