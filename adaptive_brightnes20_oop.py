...
class BrightnessController:
    def __init__(self, camera_index=0, num_threads=4, frame_queue_size=100, brightness_queue_size=100,
        ...
        self.activity_timeout = 5*60  # 5 minutes
    ...
    def adjust_screen_brightness(self):
        ...
        try:
            ...
            # Start the activity listeners
            with mouse.Listener(on_move=self.on_move, on_click=self.on_click, on_scroll=self.on_scroll) as m_listener, keyboard.Listener(on_press=self.on_press, on_release=self.on_release) as k_listener:
    ...
    def load_state(self):
        try:
            ...
            print(f"Error loading state: {str(e)} at {datetime.now().strftime('%H:%M:%S')}")
            return None
    ...
    def save_state(self, state):
        try:
            ...
            print(f"Error saving state: {str(e)} at {datetime.now().strftime('%H:%M:%S')}")
            return None


if __name__ == "__main__":
    bc = BrightnessController()
    bc.adjust_screen_brightness()
