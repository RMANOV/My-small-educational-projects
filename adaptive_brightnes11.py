    ...
    while True:
        try:
            ...
            if screenshot_brightness is not None:
                ...
                print(f'Checking screenshot brightness: {screenshot_brightness}% at {
        ...
        if time.time() - last_brightness_change_time > 300 or not self.is_system_stable() or self.is_system_sleeping() or self.is_fullscreen_mode() or screenshot_wait_count > 300:
            print(f"Exiting main loop due to inactivity or system instability at {
            ...
            while True:
                try:
                    ...
                    if self.is_user_active():
                        print(f"User activity detected. Resuming brightness control at {
                              datetime.now().strftime('%H:%M:%S')}")
                    ...
                    if screenshot_brightness is not None:
                        print(f"Screenshot brightness available. Resuming brightness control at {
                              datetime.now().strftime('%H:%M:%S')}")
                        return screenshot_brightness
                    ...
                    print(f'Error during sleep mode: {str(e)} at {
                          datetime.now().strftime("%H:%M:%S")}')
                    wait_count += 1
